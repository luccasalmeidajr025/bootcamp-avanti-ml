...
for **row** in range(2):
  for **col** in range(2):
    axs[row, col].plot(np.arange(10), np.random.rand(10))
    axs[row, col].set_title(f"Subplot {row + 1}, {col + 1}")
    ...
    axs[row, col].annotate(f'axs[{row}, {col}]', (0.5, 0.5),
                          ...
                          color='darkgrey')
fig.suptitle('**matplotlib.pyplot**.subplots()')```

As palavaras faltantes são: matplotlib, np, row, col, fontsize,matplotlib.pyplot
...
As palavras faltantes são: matplotlib, matplotlib.pyplot,linspace,sin,fig,ax, x e y
